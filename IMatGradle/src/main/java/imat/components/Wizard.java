...
public class Wizard extends AnchorPane implements Observer {
    ...
    private Wizard(ShoppingState observable) {
        ...
        try {
            root.load();
            observable.getAuthenticatedUser().ifPresentOrElse(u -> billingPane = BillingPane.getInstance(u.billingInformation()), () -> billingPane = BillingPane.getInstance());
            checkoutPane = CheckoutPane.getInstance();
            paymentPane = PaymentPane.getInstance();
